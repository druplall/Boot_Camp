...
# Import to know, from the server point of view: GET is used to send back data only and POST is used to receive data
...
def get_store(name):
    ...
    # if none match, return an error meessage
    for store in stores:
        if store['name'] == name:
...
@app.route('/store')
def get_stores():
    return jsonify({'stores':stores}) # this will return a string from the list of store
